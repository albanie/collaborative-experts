    ...
    for mode in "t2v", "v2t":
        ...
        for row in log:
            if seed_tag in row:
                ...
                match = re.search(f"{seed_tag} (\d+)$", row)  # NOQA
        ...
        for metric, subdict in scores.items():
            for seed, values in subdict.items():
                if eval_mode == "test_run":
                    # All test runs should be be grouped under a single seed
                    msg = f"expected single number from test run, found {len(subdict)}"
                    assert len(subdict) == 1, msg
                    stat = values
